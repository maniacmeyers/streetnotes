...
/**
 * Idempotent seed for the ServiceNow K26 — Booth Campaign.
 *
 * - Owns campaigns where event_name = 'ServiceNow Knowledge 26 (K26)'
 *   created by 'jeff@forgetime.ai'.
 * - Wipes any existing copies (and their child files/channels via FK cascade)
 *   before re-inserting, so re-running picks up content updates.
 * - Sets status='active' so non-Jeff BDRs can see it via the Campaigns list
 *   filter (active / approved / created_by self).
 *
 * Usage: POST /api/vbrick/campaigns/seed-k26
 */

const CAMPAIGN_OWNER = 'jeff@forgetime.ai'
const CAMPAIGN_EVENT = 'ServiceNow Knowledge 26 (K26)'
...
const CAMPAIGN_DESCRIPTION =
  'Drive booth traffic and meeting conversion at ServiceNow Knowledge 26. Vbrick is the only AI-powered enterprise video platform with a certified, pre-built integration for ServiceNow. Use case focus: Now Assist video knowledge, AI Search video, ITSM workflow embedding, Employee Center Pro, Video on Demand, and Video Connector.'

const CAMPAIGN_AUDIENCE =
  'ServiceNow customers and prospects — IT leaders, ITSM managers, HR/employee experience leaders, CIO/CTO, internal communications teams — who already run ServiceNow and want to add enterprise video into their workflows, Employee Center Pro, Service Portal, and Now Assist AI.'

export async function POST() {
  const supabase = createAdminClient()

  // 1) Wipe existing K26 campaigns owned by Jeff so this is idempotent.
  //    campaign_files / campaign_channels FK cascade on campaign delete (per migration 009).
  const { data: existing } = await supabase
    .from('campaigns')
    .select('id')
    .eq('created_by', CAMPAIGN_OWNER)
    .eq('event_name', CAMPAIGN_EVENT)
  ...
  // 2) Insert the campaign as 'active' so all BDRs see it via the list filter.
  const { data: campaign, error: campaignErr } = await supabase
    ...
    .insert({
      ...
      created_by: CAMPAIGN_OWNER,
      status: 'active',
      frameworks: ['maniac_method'],
      metadata: { seeded: true, source: 'seed-k26' },
    })
    .select()
    .single()
  ...
  if (filesErr) {
    return NextResponse.json({ error: `Files insert failed: ${filesErr.message}` }, { status: 500 })
  }
  ...
  return NextResponse.json({
    ...
    filesInserted: insertedFiles?.length ?? 0,
    cleanedUp: existingIds.length,
    nextSteps: [
      'Open Campaigns in the dashboard — K26 is now visible to all BDRs.',
      `Optionally trigger AI generation: POST /api/vbrick/campaigns/${campaign.id}/generate`,
    ],
  })
}
