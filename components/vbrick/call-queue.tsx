  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <span className="text-[11px] uppercase tracking-[0.2em] text-[#7ed4f7] font-inter font-medium">
          Session — {totalCount} Calls
        </span>
        <button
          onClick={onEndSession}
          className="text-xs text-gray-400 hover:text-white font-inter uppercase tracking-wider cursor-pointer transition-colors duration-200"
        >
          End Session
        </button>
      </div>
      ...
      <div className="h-2 bg-white/10 rounded-full overflow-hidden">
        <motion.div
          className="h-full bg-[#7ed4f7] rounded-full"
          style={{ boxShadow: '0 0 8px rgba(126,212,247,0.4)' }}
          initial={{ width: 0 }}
      ...
      <p className="text-xs text-gray-500 font-fira-code">
        {completedCount} of {totalCount} completed
      </p>

      {/* Up Next card */}
      {upNext && (
        <GlassCardElevated className="border-[#7ed4f7]/30">
          <p className="text-[10px] uppercase tracking-[0.15em] text-[#7ed4f7] font-inter mb-2">
            Up Next
          </p>
          ...
          {!upNext.contact_title && (
            <p className="text-gray-400 text-sm font-inter">{upNext.company}</p>
          )}

          {upNext.phone && (
            <a
              href={`tel:${upNext.phone}`}
              className="text-[#7ed4f7] font-fira-code text-lg mt-2 inline-block cursor-pointer hover:underline"
            >
          ...
          {upNext.salesforce_notes && (
            <p className="text-gray-500 text-xs font-inter mt-2 italic">
              {upNext.salesforce_notes}
            </p>
          )}
          ...
          <button
            ...
            className="mt-3 text-gray-500 text-xs font-inter hover:text-gray-400 transition-colors cursor-pointer flex items-center gap-1"
          >
      ...
      {!upNext && completedCount > 0 && (
        <div className="text-center py-6">
          ...
          <button
            onClick={onEndSession}
            className="mt-4 px-6 py-3 rounded-lg font-bold uppercase tracking-widest text-sm cursor-pointer transition-opacity hover:opacity-90"
      ...
      {completed.length > 0 && (
        <div className="space-y-1">
          <p className="text-[10px] uppercase tracking-[0.15em] text-gray-500 font-inter mb-2">
            Completed
          </p>
          <AnimatePresence>
            {completed.map((item) => (
              ...
              >
                ...
                <span className={`font-fira-code text-sm ${item.spinScore ? scoreColorClass(item.spinScore) : 'text-gray-600'}`}>
                  {item.spinScore ? item.spinScore.toFixed(1) : '—'}
                </span>
                <span className="text-gray-500 text-xs font-fira-code">
      ...
      {remaining.length > 0 && (
        <div className="space-y-1">
          <LuminousDivider className="my-2" />
          <p className="text-[10px] uppercase tracking-[0.15em] text-gray-500 font-inter mb-2">
            Remaining
          </p>
          {remaining.map((item) => (
            ...
            >
              ...
              <span className="text-gray-500 text-xs font-inter truncate">
                {item.company}
              </span>
            </button>
          ))}
