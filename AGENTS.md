...
- **AI Structuring:** Anthropic Codex API (`@anthropic-ai/sdk`, Sonnet 4.6 via tool_use)
...
useVoiceRecorder hook → MediaRecorder (MIME negotiation) → audioBlob
  ...
  → POST /api/structure (JSON) → Codex Sonnet tool_use → CRMNote (Zod-validated)
...
- `/api/structure` uses Codex tool_use with a JSON schema to force structured output, then validates with Zod
...
- `lib/notes/input-schema.ts` — JSON Schema for Codex tool_use input
...
- `app/api/structure/route.ts` — Codex structuring endpoint (auth + tool_use + Zod validation)
...
Per-user rolling memory of entities (contacts, accounts, products, competitors) extracted from prior notes. Used to improve CRM extraction accuracy by giving Codex known-entity context.
