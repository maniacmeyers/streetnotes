...
export const K26_OWNER = 'jeff@forgetime.ai'
export const K26_EVENT = 'ServiceNow Knowledge 26 (K26)'

export const K26_DESCRIPTION =
...
/**
 * Seed (or re-seed) the ServiceNow K26 — Booth Campaign.
 *
 * Wipes any existing K26 row owned by Jeff (FK cascade cleans up child
 * files/channels), then inserts a fresh campaign with status='active'
 * so all BDRs see it via the campaigns list filter (active/approved/
 * own).
 *
 * Idempotent — re-runnable any time to refresh content.
 */
export async function seedK26Campaign(
  supabase: SupabaseClient,
): Promise<SeedK26Result> {
  const errors: string[] = []
  ...
  if (existingIds.length > 0) {
    ...
    if (delErr) {
      ...
      return { campaignId: null, filesInserted: 0, cleanedUp: 0, errors }
    }
  }
  ...
  const { data: campaign, error: campaignErr } = await supabase
    ...
    .insert({
      ...
      description: K26_DESCRIPTION,
      event_name: K26_EVENT,
      target_audience: K26_AUDIENCE,
      created_by: K26_OWNER,
      status: 'active',
  ...
  if (campaignErr || !campaign) {
    errors.push(`campaign-insert: ${campaignErr?.message || 'unknown'}`)
    return { campaignId: null, filesInserted: 0, cleanedUp: existingIds.length, errors }
  }

  const fileRecords = K26_FILES.map((f) => ({
    campaign_id: campaign.id,
    file_name: f.file_name,
    file_type: f.file_type,
    file_size: f.file_size,
    extracted_text: f.extracted_text,
    uploaded_by: K26_OWNER,
  }))
  ...
  if (filesErr) {
    errors.push(`files-insert: ${filesErr.message}`)
  }
